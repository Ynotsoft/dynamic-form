...
function GridMenu({ list, setFilter, customFiltersRenderer }) {
    ...
    if (selectedFilter && selectedFilter.header) {
      ...
      setFilter((prev) => ({
        ...
        [selectedFilter.header]: {
          ...
          operator:
            selectedFilter.type === "Date"
              ? selectedFilter.operator ? selectedFilter.operator : "between"
              : selectedFilter.type === "Checkbox"
                ? "in"
                : localOperator,
          source: searchForm[selectedFilter.header].source,
          displayValue:
            selectedFilter.type === "Date" && (localValue.startDate && localValue.endDate)
        ...
        <div className="absolute z-10 mt-2 w-56 bg-white rounded-md shadow-lg ring-0 ring-black ring-opacity-10 top-full left-0">
          {Object.keys(searchForm).map((key) => (
            <button
              ...
              className={`block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 w-full text-left ${selectedFilter?.header === key ? 'bg-indigo-100' : ''
  ...
  // If custom filters renderer is provided, use it
